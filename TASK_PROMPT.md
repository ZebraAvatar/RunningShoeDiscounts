...
A promotions table — one row per promotion actually read: what it says, rate, code, and the end date exactly as printed (or "no end date printed"). Include promotions whose prices had reverted, so the record is complete.
An alert box, three lines maximum — best find, then the one structural fact affecting many rows, then anything that vanished.
