...
Never name the expected answer in an extraction prompt. Ask "quote any promotional text verbatim," never "does it show the 20% badge?"
Never carry a value forward. Prior state is for diffing only.
Qualify on the visible price. Codes and cart discounts annotate a row; they never put one on the board.
Fail loud. An unreadable field is "could not read," never a blank and never a default.
Report what's listed. Don't adjudicate whether a sale is really on — that isn't observable and isn't the job.
Split of labour
...
`.github/workflows/daily-board.yml` runs the board daily at 04:00 UTC and commits the
result as `index.html`, which GitHub Pages serves. The commit uses Actions' built-in
`GITHUB_TOKEN` — there is no PAT anywhere in this setup.
One secret is required, under Settings → Secrets and variables → Actions:
`CLAUDE_CODE_OAUTH_TOKEN` — bills to a Claude Pro/Max subscription. Generate it by
installing Claude Code (`npm install -g @anthropic-ai/claude-code`) and running
`claude setup-token` once. That single command is the only thing Claude Code is needed for.
Or `ANTHROPIC_API_KEY` — bills to the API instead. Swap the `with:` key in the workflow.
Pages: Settings → Pages → Deploy from a branch → `main` → `/ (root)`.
Pages on a private repo requires GitHub Pro; on a free account the repo must be public.
`TASK_PROMPT.md` stays the single source of truth for how a run behaves. The workflow
only overrides where the board is written.
